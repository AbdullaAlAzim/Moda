...
?>
<div class="wishlit-product-wraper">
    <div class="cart-list">
        <!-- WISHLIST TABLE -->
        <table
                class="shop_table table cart wishlist_table wishlist_view traditional responsive <?php echo wp_kses_post($no_interactions) ? 'no-interactions' : ''; ?> <?php echo wp_kses_post($enable_drag_n_drop) ? 'sortable' : ''; ?> "
			...
			<span class="nobr">
				<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_name_heading', __('Product name', 'moda'), $wishlist)); ?>
			</span>
                </th>
                <th class="status"></th>
                <?php if ($show_price || $show_price_variations) : ?>
                    ...
                    <th class="product-price">
                        <span class="nobr">
                            <?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_price_heading', __('Price', 'moda'), $wishlist)); ?>
                        </span>
                    </th>
                <?php endif; ?>
				...
				<span class="nobr">
					<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_quantity_heading', __('Quantity', 'moda'), $wishlist)); ?>
				</span>
                    </th>
                <?php endif; ?>
				...
				<span class="nobr">
					<?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_stock_heading', __('Stock status', 'moda'), $wishlist)); ?>
				</span>
                    </th>
                <?php endif; ?>
                ...
                <?php if ($show_last_column) : ?>
                    <?php $column_count++; ?>
                    <th class="product-add-to-cart" colspan="2">
                        <span class="nobr">
                            <?php echo esc_html(apply_filters('yith_wcwl_wishlist_view_cart_heading', 'Action', $wishlist)); ?>
                        </span>
            ...
            if ($wishlist && $wishlist->has_items()) :
                foreach ($wishlist_items as $item) :
                    ...
                    if ($product && $product->exists()) :
                        ?>
                        <tr id="yith-wcwl-row-<?php echo esc_attr($item->get_product_id()); ?>"
                            ...
                            <?php if ($item->get_product()) : ?>
                                <td>
                                    <?php echo '<a class="quick" href="' . esc_url(get_permalink($item->get_product_id())) . '">' . esc_html(__('Quick View', 'moda')) . '</a>' ?>
                                </td>
                            <?php endif ?>
                            ...
                            <?php if ($show_stock_status) : ?>
                                <td class="product-stock-status stock">
                                    <?php do_action('yith_wcwl_table_before_product_stock', $item, $wishlist); ?>

                                    <?php echo 'out-of-stock' === $stock_status ? '<span class="wishlist-out-of-stock">' . esc_html(apply_filters('yith_wcwl_out_of_stock_label', __('Out of stock', 'moda'))) . '</span>' : '<span class="wishlist-in-stock">' . esc_html(apply_filters('yith_wcwl_in_stock_label', __('In Stock', 'moda'))) . '</span>'; ?>
                            ...
                            <?php if ($show_remove_product) : ?>
                                <td class="product-remove remove-btn">
                                    <div>
                                        <a href="<?php echo esc_url($item->get_remove_url()); ?>"
                                           class="remove remove_from_wishlist"
                                           title="<?php echo esc_html(apply_filters('yith_wcwl_remove_product_wishlist_message_title', __('Remove this product', 'moda'))); ?>"><span><i
                                                        class="fal fa-trash-alt"></i></span></a>
                                    </div>
                                </td>
                            <?php endif; ?>

                            <?php if ($show_last_column) : ?>
                                <td class="product-add-to-cart shop-btn">
                                    ...
                                    if ($show_dateadded && $item->get_date_added()) :
                                        ...
                                        echo '<span class="dateadded">' . esc_html(sprintf(__('Added on: %s', 'moda'), $item->get_date_added_formatted())) . '</span>';
                                    endif;
                                    ?>
                                    ...
                                    <?php if ($move_to_another_wishlist && $available_multi_wishlist && count($users_wishlists) > 1) : ?>
                                        <?php if ('select' === $move_to_another_wishlist_type) : ?>
                                            <select class="change-wishlist selectBox">
                                                <option value=""><?php esc_html_e('Move', 'moda'); ?></option>
                                                <?php
                                        ...
                                        <?php else : ?>
                                            <a href="#move_to_another_wishlist" class="move-to-another-wishlist-button"
                                               data-rel="prettyPhoto[move_to_another_wishlist]">
                                                <?php echo esc_html(apply_filters('yith_wcwl_move_to_another_list_label', __('Move to another list &rsaquo;', 'moda'))); ?>
                                            </a>
                                        <?php endif; ?>
                                    ...
                                    <?php if ($repeat_remove_button) : ?>
                                        <a href="<?php echo esc_url($item->get_remove_url()); ?>"
                                           ...
                                           title="<?php echo esc_html(apply_filters('yith_wcwl_remove_product_wishlist_message_title', __('Remove this product', 'moda'))); ?>"><?php esc_html_e('Remove', 'moda'); ?></a>
                                    <?php endif; ?>
                ...
                <tr>
                    <td colspan="<?php echo esc_attr($column_count); ?>"
                        class="wishlist-empty"><?php echo esc_html(apply_filters('yith_wcwl_no_product_to_remove_message', __('No products added to the wishlist', 'moda'), $wishlist)); ?></td>
                </tr>
            <?php
            endif;
