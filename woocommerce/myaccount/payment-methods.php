                        ...
                        <td class="woocommerce-PaymentMethod woocommerce-PaymentMethod--<?php echo esc_attr($column_id); ?> payment-method-<?php echo esc_attr($column_id); ?>"
                            ...
                            if (has_action('woocommerce_account_payment_methods_column_' . $column_id)) {
                                ...
                                if (!empty($method['method']['last4'])) {
                                    ...
                                    echo sprintf(esc_html__('%1$s ending in %2$s', 'moda'), esc_html(wc_get_credit_card_type_label($method['method']['brand'])), esc_html($method['method']['last4']));
                                } else {
...
<?php else : ?>
    ...
    <p class="woocommerce-Message woocommerce-Message--info woocommerce-info"><?php esc_html_e('No saved methods found.', 'moda'); ?></p>
...
<?php if (WC()->payment_gateways->get_available_payment_gateways()) : ?>
    <a class="button"
       href="<?php echo esc_url(wc_get_endpoint_url('add-payment-method')); ?>"><?php esc_html_e('Add payment method', 'moda'); ?></a>
<?php endif; ?>
